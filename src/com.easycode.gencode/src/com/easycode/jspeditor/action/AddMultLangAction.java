...
import com.easycode.jspeditor.editor.Constants;
import com.easycode.jspeditor.editor.JSPMultiPageEditor;
import com.easycode.jspeditor.editor.MultLangService;
...
public class AddMultLangAction extends Action
{
    ...
    /*
     * private String readKeyFromCommProp(String txt) { if (txt == null) {
     * return null; } if (config.getCommonLangProp() != null &&
     * !"".equalsIgnoreCase(config.getCommonLangProp().trim())) { String
     * common[] = config.getCommonLangProp() .split(";");
     * 
     * for (String t : common) { if (project.getFile(t).exists()) { try {
     * PropertyResourceBundle p = new PropertyResourceBundle(
     * project.getFile(t).getContents()); Enumeration<String> enu = p.getKeys();
     * 
     * while (enu.hasMoreElements()) { String temp = enu.nextElement(); if
     * (p.getString(temp.trim()).equals(txt.trim())) { return temp; } }
     * 
     * } catch (Exception e) { // TODO Auto-generated catch block
     * e.printStackTrace(); } } else { MessageBox box = new MessageBox(new
     * Shell(), SWT.APPLICATION_MODAL | SWT.ICON_WARNING);
     * 
     * box.setText("WARNING");
     * 
     * box.setMessage(MultLang.getMultLang("code.084") + t);
     * 
     * box.open();
     * 
     * // return; } } } return null; }
    ...
    public void run()
    {
        ...
        if (part instanceof JSPMultiPageEditor)
        {
            ...
            /*
             * String[] tempPath =
             * f.getFile().getProjectRelativePath().toString().split("/");
             * if(tempPath.length > 3) { suff = tempPath[2]+""; } else
             * if(tempPath.length > 2) { suff = tempPath[1]+""; }
             */
            ...
            for (PosObject p : posList)
            {
                try
                {
                    // doc.replace(p.getSrcBeginPos()+addSize,
                    // p.getSrc().length(), p.getReplace());
