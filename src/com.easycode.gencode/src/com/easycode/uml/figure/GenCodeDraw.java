...
/**
 * 功能描叙: 编 码: ouyangchao 完成时间: 2012-6-1 下午10:19:16
 */
		...
		if (fieldList != null)
		{
			...
			for (Map m : fieldList)
			{
				...
				else
				{
					...
					//attr = new Label((String) m.get("propName"), publicImage);
				}
				attr.setToolTip(new Label(attr.getText()));
				//attr.setBorder(new LineBorder(ColorConstants.green, 1));
				String isObj = String.valueOf(type.get("isObject"));
				//System.err.println("----add---->"+propName);
				...
				//String refer = "";
					...
					if ("true".equals(isObj))
					{
					    ...
					    if("true".equals((String)type.get("array")))
					    {
	                        ...
	                        if(tmpClzName.indexOf("[")>-1)
	                        {
	                        	...
	                        	//System.err.println("-------xxxxxxxxxxxxxx-->"+referClzName);
	                        }
						    //referMap.put((String) m.get("propName"), "one-to-more");
							refer = "one-to-more";
					    }
						}
					}
					else
					{
						//continue;
						/*
						String referObj = referClzName = (String) m
								.get("referObj");
						if (referObj.endsWith(".class"))
						{
							referClzName = referObj.substring(0, referObj
									.lastIndexOf("."));
					...
					//System.err.println(type.get("array"));
		...
		if (fileList != null)
		{
			...
			for (FileModel fm : fileList)
			{
				...
				if (fm.getFilePath() != null)
				{
					...
					if (memo != null)
					{
						...
						for (String m : memos)
						{
							...
							if (name != null && !"".equals(name))
							{
								...
								//System.err.println("创建锚:(" + name + "." + i
								//		+ ")");
							}
							if(memoParse.length == 3)
							{
								//System.err.println("保存:"+name + "." +  memoParse[2]);
					...
					//System.err.println("drawqqqq:"+fileName);
