...
public class DbMgr extends ConnectionBase
{
    ...
    public static TableModel getTableInfo(DB db, String table)
    {
        ...
        try
        {
            ...
            try
            {
                conn = getCon(db.getDriver(), db.getUrl(), db.getUsername(), db.getPassword());
            }
            catch (Exception e)
            {
                // TODO Auto-generated catch block
            ...
            while (resultSet.next())
            {
                ...
                if (tableName.equalsIgnoreCase(table))
                {
                    ...
                    while (rs.next())
                    {
                        ...
                        if (db.getDriver().toUpperCase().indexOf("MYSQL") > -1)
                        {
                            c.setIsAutoInc(rs.getString("IS_AUTOINCREMENT"));
                        }
                        else if (db.getDriver().toUpperCase().indexOf("ORACLE") > -1)
                        {
                            c.setIsAutoInc("NO");
                        }
                        modelList.add(c);
